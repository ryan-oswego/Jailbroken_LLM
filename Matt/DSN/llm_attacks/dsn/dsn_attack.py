...
def token_gradients_dsn_loss(model, input_ids, input_slice, target_slice, loss_slice, test_prefixes, test_token_length, test_prefixes_toks, alpha, use_decay=False, use_refusal=False):
    ...
    print("[DEBUG] one_ho.is_leaf:", one_hot.is_leaf)
    print("[DEBUG] one_hot.requires_grad:", one_hot.requires_grad)
    print("[DEBUG] one_hot sum:", one_hot.sum().item())
    ...
    decoded = tokenizer.decode(pred_ids[0])
    print("[DEBUG] Model output (greedy decode):", decoded)
    ...
    print("[DEBUG] Affirmative loss:", affirmative_loss.item())

    if not use_refusal:
        loss = affirmative_loss
        print("[DEBUG] DSN loss (affirmative only):", loss.item())
    else:
        refusal_loss = 0
        count_loss = 0
        ...
        print("[DEBUG] Refusal loss:", refusal_loss.item())
        print("[DEBUG] Combined DSN loss:", loss.item())

    loss.backward()
    
    print("[DEBUG] input_embeds.grad.sum():", input_embeds.grad.sum().item())
    print("[DEBUG] input_embeds.grad.abs().sum():", input_embeds.grad.abs().sum().item())

    grad_in_embed_space = input_embeds.grad.squeeze(0)
    grad_token_space = grad_in_embed_space @ embed_weights.T

    print("[DEBUG] grad_token_space shape:", grad_token_space.shape)
    print("[DEBUG] grad_token_space total:", grad_token_space.abs().sum().item())
    print("[DEBUG] grad_token_space row-wise:", grad_token_space.abs().sum(dim=1))

    return grad_token_space.clone()
    ...
    def step(self,
        ...
        for j, worker in enumerate(self.workers):
            ...
            if grad.shape != new_grad.shape:   
                ...
                print(f"[DEBUG] grad in MPA(if): {grad}")
            else:
                grad += new_grad
                print(f"[DEBUG] grad in MPA(else): {grad}")
        ...
        print(f"[DEBUG] loss in MPA: {loss}")
        print(f"[DEBUG] refusal_loss in MPA: {refusal_loss}")
